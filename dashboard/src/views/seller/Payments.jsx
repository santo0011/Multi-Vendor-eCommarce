    ...
    const Row = ({ index, style }) => {
        return (
            <div style={style} className='flex text-sm'>
                ...
                <div className='w-[25%] p-2 whitespace-nowrap'>{moment(pendingWithdrows[index]?.createdAt).format('L')}</div>
    ...
    const Rows = ({ index, style }) => {
        return (
            <div style={style} className='flex text-sm'>
                ...
                <div className='w-[25%] p-2 whitespace-nowrap'>{moment(successWithdrows[index]?.createdAt).format('L')}</div>
                    ...
                    <div className='py-5'>
                        <form onSubmit={sendRequest}>
                            <div className='flex gap-3 flex-wrap'>
                                <input onChange={(e) => setAmount(e.target.value)} required value={amount} placeholder='Enter amount' min='0' type="number" className='px-3 md:w-[79%] py-2 focus:border-indigo-500 outline-none bg-[#283046] border border-slate-700 rounded-md text-[#d0d2d6]' name='amount' />
