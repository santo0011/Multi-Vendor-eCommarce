...
const ChatSeller = () => {
    ...
    const { sellers, seller_admin_message, successMessage, currentSeller, activeSellers } = useSelector(state => state.chat)
                    ...
                    <div className={`w-[280px] h-full absolute z-10 ${show ? '-left-[16px]' : '-left-[336px]'} md:left-0 md:relative transition-all`}>
                        <div className='w-full h-[calc(100vh-177px)] bg-[#252b3b] md:bg-transparent overflow-y-auto'>
...

                            {
                                sellers?.map((s, i) => {
                                    return <>
                                        <Link key={i} to={`/admin/dashboard/chat-sellers/${s._id}`} className={`h-[60px] flex justify-start gap-2 items-center text-white px-2 py-2 rounded-sm cursor-pointer ${sellerId === s._id ? 'bg-slate-700' : ''}`}>
                                            <div className='relative'>
                                                <img className='w-[38px] h-[38px] border-white border-2 max-w-[38px] p-[2px] rounded-full' src={s.image} alt="sellerImg" />
                                                {
                                                    activeSellers.some((a, i) => a.sellerId === s._id) &&
                        ...
                        <div className='flex justify-between items-center'>
                            {
                                sellerId && <div className='flex justify-start items-center gap-3'>
                                    <div className='relative'>
                                        <img className='w-[42px] h-[42px] border-green-500 border-2 max-w-[38px] p-[2px] rounded-full' src={currentSeller.image} alt="currentImage" />
                                        {
                                            activeSellers.some((a, i) => a.sellerId === sellerId) &&
                                            <div className='w-[10px] h-[10px] bg-green-500 rounded-full absolute right-0 bottom-0'></div>
                                        }
                                    </div>
